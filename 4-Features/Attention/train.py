...
class TransformerClassifier(nn.Module):
    def __init__(self, input_dim, n_heads, num_encoder_layers, hidden_dim, num_classes):
...
def train(x, y, train_flag=True):
    ...
    model = TransformerClassifier(input_dim=input_size, n_heads=4, num_encoder_layers=5, 
                                  hidden_dim=64, num_classes=2).to(device)
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            predict_label.extend(predicted.view(-1).tolist())
